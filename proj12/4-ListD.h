...
class ListD
{
 ...
 private:
   ...
   */
   ...
   Returns the pointed to the node with the smallest item after start
   */
   doubleNode<T>* FindSmall(int start, doubleNode<T>* startNode);

   /*
   Helper function for sort.
   Swaps the items between nodes pointed to by start and smallest
   */
   void Swap(doubleNode<T>* startNode, doubleNode<T>* smallest); 
