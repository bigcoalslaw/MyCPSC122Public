...
class ListT
{
 ...
 private:
   ...
   */
   ...
   /*
   pre: an instance of List exists and is not empty
   post: the pointer to node at the desired position is returned
   */
   node* PtrToAny(int pos);

   /*
   pre: an instance of List exists and is not empty
   post: the node at the desired position is deleted
   */
   void DeleteItemHelper(int pos);
